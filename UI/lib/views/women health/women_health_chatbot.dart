...
import '../../core/services/women health chat service/chat_service.dart';
import '../../view model/women health view model/women_health_chat_history_provider.dart';
import '../../view model/women health view model/women_health_chat_id_provider.dart';

class WomenHealthChatbot extends StatefulWidget {
  const WomenHealthChatbot({super.key});

  @override
  State<WomenHealthChatbot> createState() => _ChatBotState();
}

class _ChatBotState extends State<WomenHealthChatbot> {
  ...
  Future<void> _sendMessage(BuildContext context) async {
    if (_messageHasData && !_isLoading) {
      ...
      final chatIdProvider =
          Provider.of<WomenHealthChatIdProvider>(context, listen: false);
      final chatHistoryProvider =
          Provider.of<WomenHealthChatHistoryProvider>(context, listen: false);

      // Add the user's message to the provider and display it
      chatHistoryProvider.addwomenHealthsMessage(chatMessage);
      ...
      final womenHealthChatServices =
          WomenHealthChatService(chatIdProvider, chatHistoryProvider);
      await womenHealthChatServices.askAssitant(
        {
  ...
  Widget build(BuildContext context) {
    final chatHistoryProvider =
        Provider.of<WomenHealthChatHistoryProvider>(context);
    List<ChatMessage> chatHistory = chatHistoryProvider.womenHealthChatHistory;
        ...
        flexibleSpace: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              SizedBox(
                height: 30.h,
                child: Image.asset('assets/images/human-female.png'),
              ),
            ],
          ),
        ),
